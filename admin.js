...
// About management (admin) elements
const manageAboutBtn = document.getElementById("manageAboutBtn");
const aboutModal = document.getElementById("aboutModal");
const aboutForm = document.getElementById("aboutForm");
const closeAboutModal = document.getElementById("closeAboutModal");
const addAboutPostBtn = document.getElementById("addAboutPostBtn");
const aboutPostForm = document.getElementById("aboutPostForm");
const aboutPostEditId = document.getElementById("aboutPostEditId");
const aboutPostType = document.getElementById("aboutPostType");
const aboutPostUrl = document.getElementById("aboutPostUrl");
const aboutPostText = document.getElementById("aboutPostText");
const saveAboutPostBtn = document.getElementById("saveAboutPostBtn");
const cancelAboutPostBtn = document.getElementById("cancelAboutPostBtn");
const aboutPostsContainer = document.getElementById("aboutPosts");
...
/* About (page + posts) persistence in localStorage for quick admin edits */
function _aboutStorageKey() {
  return "jmbarber_about_items";
}

function loadAboutPosts() {
  try {
    const raw = localStorage.getItem(_aboutStorageKey()) || "[]";
...
function saveAboutPosts(posts) {
  localStorage.setItem(_aboutStorageKey(), JSON.stringify(posts));
}

function renderAboutPosts() {
  const posts = loadAboutPosts();
  if (!aboutPostsContainer) return;
  if (posts.length === 0) {
    aboutPostsContainer.innerHTML = `<div class="empty">No hay publicaciones.</div>`;
    return;
  }
  aboutPostsContainer.innerHTML = posts.map((post) => {
    let preview = "";
    if (post.type === "image") preview = `<img src="${post.content}" alt="" style="max-width:200px;border-radius:6px;display:block;margin-bottom:8px">`;
    if (post.type === "video") preview = `<div style="background:#000;padding:8px;border-radius:6px;color:#fff;max-width:320px">Video: <a href="${post.content}" target="_blank" style="color:#f2c177">Abrir</a></div>`;
    if (post.type === "text") preview = `<div class="muted" style="white-space:pre-wrap">${post.content}</div>`;
    return `
      <div class="request-item" data-id="${post.id}">
        <div>
        ...
        <div class="request-actions">
          ...
          <button class="btn danger" data-action="delete" data-id="${post.id}">Eliminar</button>
        </div>
      </div>`;
  }).join("");

  // attach handlers
  aboutPostsContainer.querySelectorAll('[data-action="edit"]').forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const id = e.currentTarget.dataset.id;
      openPostEditor(id);
    });
  });
  aboutPostsContainer.querySelectorAll('[data-action="delete"]').forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      const id = e.currentTarget.dataset.id;
      const posts = loadAboutPosts().filter((p) => String(p.id) !== String(id));
      saveAboutPosts(posts);
      renderAboutPosts();
...
function closeAboutModalFn() {
  ...
  aboutPostForm?.classList.add("hidden");
}

function openPostEditor(editId = null) {
  if (!aboutPostForm) return;
  aboutPostForm.classList.remove("hidden");
  if (!editId) {
    aboutPostEditId.value = "";
    aboutPostType.value = "image";
    aboutPostUrl.value = "";
    aboutPostText.value = "";
    return;
  }
  ...
  if (!post) return;
  aboutPostEditId.value = post.id;
  aboutPostType.value = post.type;
  if (post.type === "text") {
    aboutPostText.value = post.content;
    aboutPostUrl.value = "";
  } else {
    aboutPostUrl.value = post.content;
    aboutPostText.value = "";
  }
}
...
function savePostFromForm() {
  const type = aboutPostType.value;
  const url = aboutPostUrl.value && aboutPostUrl.value.trim();
  const text = aboutPostText.value && aboutPostText.value.trim();

  if (type !== "text" && !url) {
    showToast("Proporciona una URL válida para imagen o video.");
    return;
  }

  if (type === "text" && !text) {
    showToast("Escribe el texto de la publicación.");
    return;
  }

  const posts = loadAboutPosts();
  const editId = aboutPostEditId.value;
  if (editId) {
    const idx = posts.findIndex((p) => String(p.id) === String(editId));
    if (idx !== -1) {
      posts[idx].type = type;
      posts[idx].content = type === "text" ? text : url;
...
function loadAboutPageData() {
  try {
    const raw = localStorage.getItem("jmbarber_about_page") || "{}";
    const data = JSON.parse(raw);
    if (!aboutForm) return;
    ['aboutHeroTitle','aboutHeroText','aboutHeroImage','aboutStoryTitle','aboutStoryText','aboutPromoTitle','aboutPromoText','aboutGallery1','aboutGallery2','aboutGallery3'].forEach((id) => {
...
function saveAboutPageData() {
  ...
  localStorage.setItem('jmbarber_about_page', JSON.stringify(data));
  showToast('Contenido de Sobre MI guardado.');
}
...
async function initAdmin() {
  ...
  // About management handlers
  if (manageAboutBtn) manageAboutBtn.addEventListener("click", () => {
    loadAboutPageData();
    renderAboutPosts();
    openAboutModal();
  });
  if (closeAboutModal) closeAboutModal.addEventListener("click", closeAboutModalFn);
  if (addAboutPostBtn) addAboutPostBtn.addEventListener("click", () => openPostEditor());
  if (saveAboutPostBtn) saveAboutPostBtn.addEventListener("click", savePostFromForm);
  if (cancelAboutPostBtn) cancelAboutPostBtn.addEventListener("click", cancelPostEditor);
